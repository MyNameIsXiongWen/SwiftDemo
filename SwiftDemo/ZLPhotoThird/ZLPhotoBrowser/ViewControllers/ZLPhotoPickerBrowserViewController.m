...
- (UICollectionView *)collectionView{
    if (!_collectionView) {
        ...
        NSString *v = [NSString stringWithFormat:@"V:|-(-%@)-[_collectionView]-(-84)-|",(self.isPush) ? @"0" : @"84"];
        [self.view addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:v options:0 metrics:nil views:@{@"_collectionView":_collectionView}]];
...
- (UIButton *)deleleBtn{
    if (!_deleleBtn) {
        ...
        NSDictionary *metrics = @{@"deleteBtnWH":@(0),@"margin":@(0)};
...
-(void)setPageLabelPage:(NSInteger)page{
    ...
    if (self.isPush) {
        self.navigationItem.title = self.pageLabel.text;
    }
}
...
- (void)showPushPickerVc:(UIViewController *)vc{
    ...
    if (weakVc != nil) {
        if (([vc isKindOfClass:[UITableViewController class]] || [vc isKindOfClass:[UICollectionView class]]) && weakVc.navigationController != nil && self.navigationHeight == 0) {
            ...
            //            = CGRectGetMaxY(weakVc.navigationController.navigationBar.frame);
        }
        self.navigationHeight = 0;
        [weakVc.navigationController pushViewController:self animated:NO];
    }
}
